...
{
    $hover_text = '<b>' . sprintf( __( 'Only works with %1$sPro Print Service%2$s', 'print-my-blog' ), '<a href="' . pmb_fs()->get_upgrade_url() . '">', '</a>' ) . '</b>';
    if ( $explanation ) {
...
{
    $hover_text = '<b>' . sprintf( __( 'Works better with %1$sPro Print Service%2$s', 'print-my-blog' ), '<a href="' . pmb_fs()->get_upgrade_url() . '">', '</a>' ) . '</b>';
    if ( $explanation ) {
        $hover_text .= "<br>" . $explanation;
    }
...
{
    $hover_text = '<b>' . sprintf( __( 'More Advanced Features in %1$sPro Print%2$s', 'print-my-blog' ), '<a href="' . admin_url( PMB_ADMIN_PROJECTS_PAGE_PATH ) . '">', '</a>' ) . '</b>';
    if ( $explanation ) {
        $hover_text .= "<br>" . $explanation;
    }
...
{
    return pmb_render_template( 'partials/design_preview.php', [
        'design' => $design,
    ] );
}
...
function pmb_content_item( $posty_row, Project $project, $max_nesting = null )
{
    ...
    if ( $post_type->public ) {
        ...
        ?>"
                                    title="<?php 
        echo  esc_attr( sprintf( __( 'View "%s"', 'print-my-blog' ), $title ) ) ;
        ?>"
    ...
    ?>"
                            title="<?php 
    echo  esc_attr( sprintf( __( 'Edit "%s"', 'print-my-blog' ), $title ) ) ;
    ?>"
    ...
    ?>
                        <a
                                title="<?php 
    echo  esc_attr( sprintf( __( 'Add "%s" to project', 'print-my-blog' ), $title ) ) ;
    ?>"
                                class="pmb-add-item" href="#pmb-content-item-<?php 
    echo  esc_attr( $id ) ;
    ?>"><span class="dashicons dashicons-plus pmb-icon pmb-clickable"></span></a>
                        <a
                                title="<?php 
    echo  esc_attr( sprintf( __( 'Remove "%s"', 'print-my-blog' ), $title ) ) ;
    ?>"
