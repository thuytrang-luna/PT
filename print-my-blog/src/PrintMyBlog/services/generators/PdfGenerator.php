...
class PdfGenerator extends ProjectFileGeneratorBase
{
    ...
    protected function generateSection()
    {
        ...
        if ( $post->pmb_section instanceof ProjectSection ) {
            ...
            if ( $template ) {
                ...
                $this->writeDesignTemplateInDivision( pmb_map_section_to_division( $post->pmb_section ) );
            }
     ...
     * @param array $context
     */
    protected function writeTemplateToFile( $template_file, $context = array() )
    {
        $this->getFileWriter()->write( '<!-- pmb template: ' . $template_file . '-->' . $this->getHtmlFrom( $template_file, $context ) );
    }
     ...
     * @param array $context
     */
    protected function writeDesignTemplateInDivision( $division, $beginning = true, $context = array() )
    {
        $this->writeTemplateToFile( $this->design->getDesignTemplate()->getTemplatePathToDivision( $division, $beginning ), $context );
    }
    ...
    protected function maybeGenerateDivisionStart( ProjectSection $last_section = null, ProjectSection $current_section = null )
    {
        ...
        if ( $last_section_placement !== $current_section->getPlacement() && $this->design->getDesignTemplate()->supports( $current_section->getPlacement() ) ) {
    ...
    protected function maybeGenerateDivisionEnd( ProjectSection $previous_section = null, ProjectSection $current_section = null )
    {
        ...
        while ( $iterate_depth >= $current_depth ) {
            $this->writeClosingForDesignTemplate( pmb_map_section_to_division( $previous_section ) );
            $iterate_depth--;
        }
