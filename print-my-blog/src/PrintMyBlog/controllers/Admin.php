...
class Admin extends BaseController
{
    ...
    public function settingsPage()
    {
        ...
        if ( $saved ) {
            ...
            if ( $posts ) {
                ...
                $text .= ' ' . sprintf( esc_html__( 'You should see the changes on your %1$slatest post%2$s.', 'print-my-blog' ), '<a href="' . $permalink . '" target="_blank">', '</a>' );
            }
    ...
    protected function getGithubHelpForm()
    {
        return new FormSection( [
            'subsections' => [
            'explanatory_text' => new FormSectionHtml( '<h2>' . __( 'Support for your plan is offered on GitHub', 'print-my-blog' ) . '</h2>' . '<p>' . __( 'GitHub is a public forum to share your issues with the developer and other users.', 'print-my-blog' ) . '</p>' . '<p>' . sprintf( __( 'You will need a GitHub account. If you prefer to use email support please %1$spurchase a license that offers email support.%2$s', 'print-my-blog' ), '<a href="' . esc_url( pmb_fs()->get_upgrade_url() ) . '">', '</a>' ) . '</p>' ),
...
```',
    ...
    protected function editChooseDesign()
    {
        ...
        $this->renderProjectTemplate( 'design_choose.php', [
    ...
    protected function renderProjectTemplate( $template_name, $args )
    {
        ...
        if ( $args['project'] instanceof Project ) {
            ...
            $args['current_step'] = $args['project']->getProgress()->mapSubactionToStep( ( isset( $_GET['subaction'] ) ? $_GET['subaction'] : null ), ( isset( $_GET['format'] ) ? $_GET['format'] : null ) );
        } else {
            $args['steps_to_urls'] = [];
    ...
    protected function getSetupForm()
    {
        ...
        foreach ( $formats as $format ) {
            $format_options[$format->slug()] = new InputOption( $format->coloredTitleAndIcon(), $format->desc() );
        }
        $format_options['all'] = new InputOption( __( 'Both', 'print-my-blog' ), __( 'A project can be prepared to use both a Digital PDF and a Print-Ready PDF, but it will be more complex. (Eg the design for one format might support different features than the design for the other format.)', 'print-my-blog' ) );
        $default_format = null;
        ...
        return apply_filters( '\\PrintMyBlog\\controllers\\Admin::getSetupForm', new FormSection( [
            ...
            'formats' => new RadioButtonInput( $format_options, [
    ...
    protected function saveNewProject()
    {
        ...
        if ( $form->getInputValue( 'formats' ) === 'all' ) {
            $formats_to_save = array_map( function ( FileFormat $format ) {
                return $format->slug();
        ...
        $this->notification_manager->addTextNotificationForCurrentUser( OneTimeNotification::TYPE_SUCCESS, sprintf( __( 'Successfully setup the project "%s".', 'print-my-blog' ), $this->project->getWpPost()->post_title ) );
        ...
        if ( $initialize_steps ) {
            ...
            foreach ( $new_formats as $new_format ) {
                ...
                $this->notification_manager->addTextNotificationForCurrentUser( OneTimeNotification::TYPE_INFO, sprintf( __( 'You need to choose and customize the design for your %s.', 'print-my-blog' ), $this->file_format_registry->getFormat( $new_format )->title() ) );
            }
        }
    ...
    protected function redirectToNextStep( Project $project )
    {
        ...
        // Redirect to it
        wp_redirect( add_query_arg( $args, admin_url( PMB_ADMIN_PROJECTS_PAGE_PATH ) ) );
        exit;
    }
    ...
    protected function saveProjectCustomizeDesign()
    {
        ...
        $this->notification_manager->addTextNotificationForCurrentUser( OneTimeNotification::TYPE_SUCCESS, sprintf( __( 'The design "%s" has been customized, and its changes will be reflected in all projects that use it.', 'print-my-blog' ), $design->getWpPost()->post_title ) );
        $this->redirectToNextStep( $this->project );
    }
    ...
    protected function saveProjectChooseDesign()
    {
        ...
        if ( !$design instanceof Design || !$format instanceof FileFormat ) {
            throw new Exception( sprintf( __( 'An invalid design (%s) or format provided(%s)', 'print-my-blog' ), Array2::setOr( $_GET, 'design', '' ), Array2::setOr( $_GET, 'format', '' ) ) );
        }
        ...
        $this->notification_manager->addTextNotificationForCurrentUser( OneTimeNotification::TYPE_SUCCESS, sprintf( __( 'You chose the design "%1$s" for the %2$s of your project.', 'print-my-blog' ), $design->getWpPost()->post_title, $format->title() ) );
        ...
        if ( isset( $_GET['page'] ) && $_GET['page'] === 'print-my-blog-projects-account' ) {
            if ( pmb_fs()->_get_license() instanceof FS_Plugin_License ) {
                try {
                    ...
                    $this->notification_manager->addTextNotificationForCurrentUser( 'warning', sprintf( __( 'There was an error communicating with Print My Blog Central. It was %s', 'print-my-blog' ), $e->getMessage() ) );
                    $this->notification_manager->showOneTimeNotifications();
                }
            }
        }
    ...
    protected function getDuplicateAsPrintMaterialHtml( $css_class = '' )
    {
        ...
        if ( $post->post_type === CustomPostTypes::CONTENT ) {
            ...
            if ( $original_id ) {
                $html = '<a href="' . esc_url( get_edit_post_link( $original_id ) ) . '" title="' . esc_attr( sprintf( __( 'Go to the %1$s "%2$s" was copied from.', 'print-my-blog' ), $type_label, $post->post_title ) ) . '"' . '>' . sprintf( esc_html__( 'Go to Original %s', 'print-my-blog' ), $type_label ) . '</a>';
            } else {
                $html = '';
            }
            ...
            if ( $print_material ) {
                $html = '<a href="' . esc_url( get_edit_post_link( $print_material->getWpPost()->ID ) ) . '" title="' . esc_attr( sprintf( __( 'Go to Print Material "%s" was created from.', 'print-my-blog' ), $print_material->getWpPost()->post_title ) ) . '"' . '>' . esc_html__( 'Go to Print Material', 'print-my-blog' ) . '</a>';
            } else {
                $html = '<a href="' . esc_url( $this->getDuplicatePostAsPrintMaterialUrl( $post ) ) . '" title="' . esc_attr( sprintf( __( 'Copy "%s" to New Print Material for a Print My Blog project', 'print-my-blog' ), $post->post_title ) ) . '"' . $css_attr . '>' . esc_html__( 'Copy to Print Material', 'print-my-blog' ) . '</a>';
            }
