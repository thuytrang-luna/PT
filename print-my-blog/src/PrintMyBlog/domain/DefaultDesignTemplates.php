...
class DefaultDesignTemplates
{
    ...
    public function getDefaultDesignForm()
    {
        return new FormSection( [
            ...
            'html_help_text'  => sprintf( __( 'Use standard %1$sCSS units%2$s', 'print-my-blog' ), '<a href="https://www.w3schools.com/CSSref/css_units.asp">', '</a>' ),
            'default'         => '8.5in',
        ] ),
    ...
    {
        $theme = wp_get_theme();
        $use_theme_help_text = sprintf( __( 'Your theme, "%1$s", can be used in conjunction with your design. Themes are often not intended for print and can conflict with the design, but you may have content that looks broken without the theme.', 'print-my-blog' ), $theme->name );
    ...
    public function getPageReferenceTextInput()
    {
        return new TextInput( [
            ...
            'validation_strategies' => [ new TextValidation( __( 'You must include "%s" in the page reference text so we know where to put the page number.', 'print-my-blog' ), '~.*\\%s.*~' ) ],
        ] );
    }
    ...
    public function getInternalFootnoteTextInput()
    {
        return new TextInput( [
            ...
            'default'               => __( 'See page %s.', 'print-my-blog' ),
            'validation_strategies' => [ new TextValidation( __( 'You must include "%s" in the footnote text so we know where to put the URL.', 'print-my-blog' ), '~.*\\%s.*~' ) ],
        ] );
    }
